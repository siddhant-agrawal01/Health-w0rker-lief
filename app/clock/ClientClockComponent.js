...
export default function ClientClockComponent() {
  ...
  const [shiftDuration, setShiftDuration] = useState(''); // Store duration as a formatted string
  ...
  useEffect(() => {
    ...
    setLocation({
      latitude: 22.95698080977593,
      longitude: 79.18895250601956,
    });
    /* Uncomment to use real geolocation once fixed
  ...
  useEffect(() => {
    ...
    return () => clearInterval(interval); // Cleanup on unmount or when shift ends
  ...
  const handleClockIn = async () => {
    ...
    try {
      ...
      for (const hospital of hospitals) {
        console.log('Hospital Coords:', parseFloat(hospital.latitude), parseFloat(hospital.longitude));
      ...
      if (!withinPerimeter) {
        setError(`You are not within the required radius of any hospital. Closest hospital radius: ${hospitals[0]?.radius || 'unknown'} km`);
        setLoading(false);
        return;
      }
