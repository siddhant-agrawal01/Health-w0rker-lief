...
export default function HistoryPage() {
  ...
  useEffect(() => {
    ...
    if (!session || session.user.role !== "CARE_WORKER") {
      ...
      fetchShifts(); // Fetch shift data for care workers
    }
  }, [session, status, router]);

  // Fetch shift data using GraphQL
            ...
            <table className="min-w-full bg-white border border-gray-200">
              <thead>
                <tr className="bg-gray-100">
                  ...
                  <th className="py-2 px-4 border-b text-left text-gray-600">Location</th>
                  <th className="py-2 px-4 border-b text-left text-gray-600">Note</th>
                </tr>
              </thead>
              <tbody>
                {shifts.map((shift) => (
                  <tr key={shift.id} className="hover:bg-gray-50 transition">
                    <td className="py-2 px-4 border-b">{new Date(shift.startTime).toLocaleString()}</td>
                    <td className="py-2 px-4 border-b">{shift.endTime ? new Date(shift.endTime).toLocaleString() : "Active"}</td>
                    <td className="py-2 px-4 border-b">{shift.latitude.toFixed(4)}, {shift.longitude.toFixed(4)}</td>
